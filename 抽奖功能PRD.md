...
| 22 | **中奖区域** | 中奖信息 | 整个区域在开奖d后显示 |
| 23 | | 中奖者 | 用户名+头像，两者均支持点击跳转用户主页 |
| 25 | | 用户标签 | 逻辑复用 |
| 24 | | 中奖时间 | Won on: [日期时间] |
| 26 | **交接展示** | 截图与留言 | 已完成交接时显示（任一方上传了截图）；标题"Prize Handover Complete!"，描述文案"Another successful giveaway on Tradekitsune!" |
| 28 | | 截图 | 可点击放大 |
| 27 | | 留言 | 最多100字符 |
| 27 | | 来自xx | "From [发奖者]/[中奖者]" |
| 30 | **推荐模块** | 推荐抽奖 | 使用抽奖列表组件；最多3条，优先展示ONGOING，其次AWAITING START，开奖时间or报名开始时间越近的展示在上方；无符合条件的（表内所有抽奖均已开奖）则隐藏整个模块 |
