...
| 2 | **发奖者信息** | 头像 | 圆形头像，可点击跳转用户主页 |
| 3 | | 用户名 | @username格式 |
| 4 | | 标签 | 最多显示8个标签 |
| 5 | | 关注按钮 | FOLLOW / FOLLOWING，根据访客关注状态显示 |
| 6 | **活动头部** | 状态标签 | AWAITING START / ONGOING / ENDED（前端仅显示这3种） |
| 7 | | 标题 | 完整展示 |
| 8 | | 描述 | 完整展示 |
| 9 | | 关联活动标签 | 如关联了运营活动则显示，标签样式，不可点击 |
| 10 | **奖品区域** | 奖品图片 | 大图展示，hover信息卡片，点击可查看道具详情弹窗 |
| 11 | | 奖品名称 | 🎁 [奖品名称] x [数量] |
| 12 | | 奖品价值 | Value: [价值] |
| 13 | | 宠物属性 | 变异、年龄、重量（仅宠物类道具显示） |
| 14 | **操作区域** | 操作按钮 | COMING SOON / JOIN NOW / JOINED / ENDED（详见6.4状态表） |
| 15 | | 参与提示 | 未参与："✓ Get notified if you win"；已参与："✓ Following @Host" |
| 16 | | 分享按钮 | SHARE，始终可见 |
| 17 | **时间信息** | 倒计时 | created:"Starts in Xd Xh Xm"；ongoing:"Ends in Xd Xh Xm"；ended:不显示 |
| 18 | | 报名时间段 | Start: [日期时间]，End: [日期时间] |
| 19 | | 开奖时间 | Draw Time: [日期时间] |
| 20 | **参与信息** | 参与人数 | "XX users joined" |
| 21 | | 参与者头像 | 最多10个头像 + "and X more"；无参与者时显示"Be the first to join!" |
| 22 | **中奖区域** | 中奖者头像 | 仅ended后显示 |
| 23 | | 中奖者用户名 | @username格式 |
| 24 | | 中奖时间 | Won on: [日期时间] |
| 25 | | 领奖截止时间 | Claim Deadline: [日期时间] |
| 26 | **交接展示** | 发奖者截图 | 仅completed且有截图时显示，可点击放大 |
| 27 | | 发奖者留言 | 选填，最多100字符 |
| 28 | | 中奖者截图 | 选填，可点击放大 |
| 29 | | 中奖者留言 | 选填，最多100字符 |
| 30 | **推荐模块** | 推荐抽奖 | 最多3条，优先ONGOING，其次AWAITING START；无符合条件则隐藏整个模块 |
