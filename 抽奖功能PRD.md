...
| 功能模块 | 功能描述 | 技术要点 |
|---------|---------|---------|
| **Lovable Cloud（或内部其他后端）** | 启用后端服务 | Supabase集成、数据库、认证系统 |
| **用户角色系统** | 安全的角色权限管理 | 独立user_roles表 |
| **道具库（已有）** | 游戏道具数据管理 | items表，包含名称、图片、价值等 |
...
#### 1. VIP抽奖发起
- **创建抽奖活动**
  - 选择道具库中的奖品（支持多个奖品，包括token或普通道具，跟创建交易中选择的方式相同）
  - 设置参与条件（默认需关注VIP）
  - 设置开奖时间（定时开奖）
  - 设置获奖人数
  - 填写活动描述
