...
- **个人主页**
  - "动态"tab中展示Giveaway列表：按开奖时间倒序，不同状态的抽奖展示对应状态标签
      - 展示该VIP创建的所有进行中抽奖和参与的抽奖（创建和参与的抽奖分别展示）
      - 长条卡片样式，包含：
        - 奖品图片
...
##### 1.2 VIP抽奖发起
- **创建抽奖活动**：拉起抽奖配置弹窗
  - 选择奖品（一次抽奖只支持1个奖品，可为token或普通道具，跟创建交易中选择方式相同）
  - 设置参与条件（本期默认只有一种条件，即关注发奖者）
  - 设置报名时间段（时间起始范围，日期+具体时间到分）
  - 开奖时间（默认逻辑为报名时间截止时自动开奖，上面设置了报名时间段后自动生成，不支持修改）
    - ⚠️ **时间限制**：报名截止时间/开奖时间不能超过创建时间起30天，以保持活动节奏
  - 填写抽奖活动标题、描述
