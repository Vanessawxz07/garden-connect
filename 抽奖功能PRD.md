...
- **参与流程**
  ...
  2. **抽奖详情页**：展示参与条件和收益说明（外显在页面上，无弹窗）
  3. **详情页点击「JOIN NOW」**：直接完成参与（无确认弹窗）
  
  - **未登录用户**：点击任意「JOIN NOW」后跳转登录/注册页面，登录完成后自动返回抽奖详情页继续参与流程
  - **已登录用户**：点击详情页「JOIN NOW」后直接参与成功

- **详情页参与信息区（JOIN NOW按钮上方外显）**
  ```
  ┌─────────────────────────────────────────┐
  │ 📋 Requirements                         │
  │ • Follow @[Creator Name] to participate │
  │                                         │
  │ 🎁 Benefits                             │
  │ • Earn [Avatar] Fan badge (14 days)     │
  │ • Notification when you win             │
  │                                         │
  │ ⚠️ Claim within 14 days after draw      │
  ├─────────────────────────────────────────┤
  │          [ JOIN NOW ]                   │
  └─────────────────────────────────────────┘
  ```
...
3. **过期处理**：到期自动将所有入口按钮变为"Expired"状态
