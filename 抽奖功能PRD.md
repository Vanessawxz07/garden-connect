...
- **创建抽奖活动**
  ...
  - 开奖时间（默认逻辑为报名时间截止时，即为开奖时间）
    - ⚠️ **时间限制**：开奖时间不能超过创建时间起30天，以保持活动节奏
  - 设置报名时间段（时间范围）
  - 填写抽奖活动标题、描述
  - **关联活动**（可选）：从下拉菜单选择运营预配置的活动名称，用于分类展示

- **查看抽奖活动**
  - 查看活动参与人数统计、活动状态等（用户中心动态tab、抽奖详情页）
  - （mvp阶段先不支持取消或编辑抽奖，后续加-报名开始之前可支持取消或修改）

#### 2. 用户参与抽奖
- **发现抽奖入口**
  - 首页顶部giveaway数量拉取入口（支持点击）、活动页（giveaway聚合页）的抽奖列表；用户主页的tab
...
##### 展示位置与规则
粉丝铭牌与用户身份标签展示在同一位置：
1. **身份标签**（如Official、Creator等）固定在第一位
2. **粉丝铭牌**按获得时间倒序排列
3. **显示上限**：
   - 个人主页：最多8个（超出不展示）
   - 其他场景：3个 + "more"按钮
...
##### 设计要点
- 视觉上需与身份标签有所区分，体现趣味性和社交属性
- 头像使设计上有辨识度，吸引其他用户点击查看
- 鼓励用户通过参与多个抽奖收集不同VIP的粉丝铭牌
...
- drawing: 正在开奖（锁定状态，不支持报名）——与研发讨论，如抽奖过程极短，不需要展示抽奖过程，则无需该状态
...
4. **入口失效**：用户中心中「确认交接」入口变为不可操作状态
5. **中奖者失去领奖资格**：不再支持领取奖品

##### 用户提示
- 在参与确认弹窗中需展示领奖有效期说明
- 在中奖通知中明确标注领奖截止时间
- 在抽奖详情页显示领奖倒计时（针对已中奖用户）
