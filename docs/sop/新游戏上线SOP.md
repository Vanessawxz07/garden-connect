...
gantt
    ...
    section 阶段一：调研准备
    游戏调研与评估       :a1, 2025-01-01, 2d
    关键信息采集         :a2, after a1, 2d
    section 阶段二：产品定义
    GameConfig配置       :b1, after a2, 1d
    页面配置清单确认     :b2, after b1, 1d
    section 阶段三：研发实现
    数据结构与导入       :c1, after b2, 2d
    页面开发与适配       :c2, after b2, 5d
    section 阶段四：运营准备
    道具数据录入         :d1, after a2, 5d
    社媒与内容准备       :d2, after c2, 2d
    section 阶段五：上线
    联调测试             :e1, after c2, 2d
    正式上线             :e2, after e1, 1d
```
...
| 交易限制条件 | 等级、会员等限制 | 无 | Gamepass限制 |

#### 2.1.3 可交易道具类型

| 调研项 | 说明 | GAG示例 | BF示例 |
|--------|------|---------|--------|
| 道具大类 | 主要道具分类 | Pets, Seeds, Gear | Fruits, Gamepasses, Swords, Guns |
| 稀有度体系 | 稀有度等级名称 | Common/Rare/Epic/Legendary/Mythic | Common/Uncommon/Rare/Legendary/Mythical |
| 特殊道具标签 | 需特殊提示的道具类型 | Mutant, Event Limited | Permanent, Physical, Limited |
...
| 价值单位 | 价值的计量单位 | Value Points / Beli |
| 价值波动频率 | 价值更新周期 | 每日/每周 |
...
```typescript
// src/config/games/{gameKey}.ts

export const bloxfruitsConfig: GameConfig = {
  // 基础信息
  gameKey: 'bloxfruits',
  ...
  itemCategories: [
    ...
    { key: 'swords', label: 'Swords', icon: '⚔️' },
    { key: 'guns', label: 'Guns', icon: '🔫' },
  ],
  ...
  // 特殊道具标签（需警告提示）
  specialItemTags: [
    { key: 'permanent', label: 'Permanent', warningText: 'Permanent fruits require careful value assessment' },
    { key: 'limited', label: 'Limited', warningText: 'Limited items may have fluctuating values' },
    { key: 'gamepass', label: 'Gamepass', warningText: 'Gamepass items have special trading rules' },
  ],
  ...
  seo: {
    ...
    keywords: ['blox fruits trading', 'blox fruits value', 'bf trade'],
  },
...
| 版本 | 日期 | 修改人 | 修改内容 |
|------|------|--------|----------|
| V1.0 | 2025-01-22 | PM | 初始版本，基于BF上线经验整理 |
