...
### 4.1 导航栏（方案定稿，视觉细化中）
视觉：https://www.figma.com/design/4mDwVh4lJ2t1Yy7EcINY5t/UI-Kit?node-id=5347-868&p=f&t=4JiE46PkLvl6yank-0
...
- **GAG已完整菜单：** 现有不变，如Trade、Value、Wiki、Calculator、Articles、Pets等
...
（可以理解为MVP阶段时，公共导航栏和GAG导航栏基本一样；但在未来阶段二多个游戏上线后，不强调GAG单游戏时，公共导航栏将改版使用下面方案二模式）
...
现网逻辑基本不变，**调整点**（待讨论，可作为P1）：未读消息数量根据导航栏/页面区分，即公共页面/公共导航栏时，则展示所有的未读消息合计；如在单游戏页面/游戏导航栏中，则展示该游戏的未读消息数。
...
```
┌──────────────────────────┐
│  👤 Personal Center      │ → 跳转 /user/{uid} 或 /user/{uid}?game=xxx
├──────────────────────────┤
│  📝 Post a GAG Trade     │ → 跳转 /growagarden/trade/create
│  📝 Post a BF Trade      │ → 跳转 /bloxfruits/trade/create
├──────────────────────────┤
│  🚪 Log Out              │ → 退出登录（可再加一个settings入口）
└──────────────────────────┘
```

| 菜单项 | 说明 |
|--------|------|
| Personal Center | 跳转当前用户的个人主页（携带游戏参数，见上方场景区分） |
...
| **GAG RESOURCES**(待定) | Trade、Trade Calculator、Pet Value、Pet Wiki、Event Wiki、Guides、Pet Management | GAG游戏功能入口 |
...
### 4.5 用户主页（MVP，待视觉细化）
- 用户基础信息不变，通过在页面内筛选游戏以切换下面的区域，展示对应游戏的tab内容。
- 需带参数以实现由其他场景进入某个游戏筛选内容的页面状态。
