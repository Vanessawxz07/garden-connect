...
## 🔄 2025年3月更新：导航栏中性化改造（A+C组合方案 → 双导航栏+动态切换）

> **更新日期：** 2025-03-04
> **更新背景：** 在实际体验中发现，用户在 Giveaway、聊天页等强游戏属性的公共页面中，如果导航栏没有对应游戏上下文，容易误以为是 bug。此前仅做了零散的小优化（根据页面内游戏 tab 选择对应 URL 参数，导航栏据此展示对应游戏导航栏），本次结合导航栏整体迭代进行全面规划。
...
经讨论，确认采用 **A+C组合方案的演化版本——双导航栏 + 动态切换**：
- 游戏专属页面和强游戏属性页面直接使用游戏导航栏
- 纯公共页面使用中性化公共导航栏
- 混合页面（Chat、User Center）根据 URL 参数动态切换

### 最终确认的导航栏显示规则

#### 核心原则
- `?game=xxx` URL 参数作为游戏上下文的 **Single Source of Truth**
- 导航栏类型由页面路径 + URL 参数共同决定

#### 完整规则表

| 页面类型 | 页面路径 | 条件 | 导航栏类型 | 说明 |
|----------|----------|------|------------|------|
| 游戏专属页 | `/{gameKey}/*` | 始终 | 游戏导航栏 | 路径本身确定游戏上下文 |
| Giveaway 列表/详情 | `/giveaways?game=xxx` | 始终有游戏上下文（默认 GAG） | 游戏导航栏 | 虽为公共路径，但始终有游戏 tab 选择，不存在无游戏上下文的空状态 |
| 聊天页 | `/chat` | 无 `?game=` 参数 | 公共导航栏 | 默认选中 All Message tab |
| 聊天页 | `/chat?game=xxx` | 有 `?game=` 参数 | 对应游戏导航栏 | 从游戏页面进入或手动切换 tab 时带入参数 |
| 用户主页 | `/user/{uid}` | 无 `?game=` 参数 | 公共导航栏 | 默认优先选中 GAG tab（影响 tab 下模块内容） |
| 用户主页 | `/user/{uid}?game=xxx` | 有 `?game=` 参数 | 对应游戏导航栏 | 从游戏页面进入时带入参数 |
| 纯公共页 | `/about-us`、`/faq`、`/privacy-policy` 等 | 无游戏属性 | 公共导航栏 | URL 不携带游戏参数 |
| 公共首页 | `/` | — | 公共导航栏 | 平台级入口 |

#### 参数来源逻辑
游戏上下文参数的产生方式：
1. **路径自带**：`/{gameKey}/*` 页面路径本身确定游戏
2. **从游戏页面进入**：如从 `/growagarden/tradelist` 点击用户头像 → `/user/{uid}?game=gag`
3. **页面内 tab 切换**：如 Giveaway 页面切换游戏 tab → URL 参数同步更新
4. **带游戏参数的公共页面链接**：如分享的 `/giveaways?game=bf` 链接

#### 导航栏切换策略：方案X（跟随参数直接切换）
- **无 `?game=` 参数时**：展示公共导航栏
- **有 `?game=xxx` 参数时**：展示对应游戏导航栏
- 参数变化时导航栏即时切换，不添加额外过渡动画
- 用户在页面内切换游戏 tab 时，URL 参数同步更新，导航栏跟随变化
...
| **All Games ▼** | 游戏选择器，下拉展示所有可用游戏，点击跳转对应游戏首页 |
| **Community ▼** | 新增一级入口，下拉菜单含：Discord、YouTube、Twitter/X、Contact |
| **Help ▼** | 新增一级入口，下拉菜单含：Beginner's Guide、Safe Trading Guide、Updatelog、FAQ、Feedback |
| **💬 MESSAGE** | 消息入口，逻辑同 4.1.4 |
| **👤 用户头像/Login** | 用户区，逻辑同 4.1.4 |
