...
##### 4.10.3.3 Beli Value校验与警告展示

**校验规则**：与创建交易页完全一致（详见4.9.5节），按场景A/B/C三档判断

**订单详情页特殊说明**：
- 场景A（纯物理价差>40%）：在订单详情页**不阻止操作**，但显示红色警告提示用户游戏内可能失败
- 场景B/C：警告逻辑与创建交易页一致

| 场景 | 订单详情页展示内容 |
|------|-------------------|
| 场景A：纯物理价差>40% | 🔴 红色警告（仅提示，不阻止）：`Beli value difference: {X}% (Max.40%) - This trade may fail in-game.` |
| 场景B：含Perm/Limited | 特殊物品警告（按提供方/接收方区分文案，见4.9.6节） |
| 场景C：含GP不含Perm/Limited，价差≤60% | 仅显示特殊物品警告 |
| 场景C：含GP不含Perm/Limited，价差>60% | 特殊物品警告 + 凑数提示 |

##### 4.10.3.4 操作按钮

- **ACCEPT TRADE**：接受交易（绿色按钮）
- 若价差>40%，按钮正常可点击，但上方显示警告提示用户游戏内可能失败
