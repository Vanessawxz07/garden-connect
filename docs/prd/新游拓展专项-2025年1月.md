...
> **调整说明（2025-01-15）：** 公共首页设计已定稿，以下为详细需求说明。
...
| 2 | GAG Top Trading Items | GAG热门交易道具（约60%占比） | 保留 |
| 3 | BF Hot Trading Items | BF热门交易道具（约20%占比） | ✅ 新增 |
| 4 | Ultimate Trading Hub | 平台介绍 + 双游戏交易数据（约20%占比） | ✅ 新增 |
| 5 | Why Choose Tradekitsune | 平台优势介绍 | 保留 |
| 6 | FAQ | 平台通用FAQ | 保留 |
| 7 | Footer | 公共Footer | 见4.2 |

#### 4.5.2 Hero区（新设计）

**整体布局**：
- 左右双栏结构，分别展示GAG和BF游戏信息
- 顶部通栏：平台标语

**顶部平台标语**：
```
主标题：Trade Roblox Game Items Safe & Fast - GAG · BF
副标题：The trusted marketplace for GAG, Blox Fruits & more
```
...
|------|------|
| 游戏标题 | "Grow a Garden" |
| 游戏描述 | "Are you ready to grow your very own garden? Plant them, wait for them to grow, and collect the profits!" |
| 适龄标签 | "Maturity: Minimal - Ages 5+" |
| Giveaways数量 | 实时统计当前进行中的GAG抽奖活动数量，数据来源：giveaways表筛选status=ongoing且为GAG游戏 |
| Ongoing Trades | 实时统计当前进行中的GAG交易数量，数据来源：订单表筛选状态为进行中 |
| Developer链接 | 可点击跳转至开发者主页，链接地址根据游戏配置，如 `https://www.roblox.com/groups/xxx` |
| 入口按钮 | 点击跳转至 `/growagarden` |
...
|------|------|
| 游戏标题 | "Blox Fruits" |
| 游戏描述 | "Welcome to Blox Fruits! Become a master swordsman or a powerful blox fruit user as you train to become the strongest player to ever live." |
| 适龄标签 | "Maturity: Mild - Ages 5+ Violence (Repeated/Mild)" |
| Ongoing Trades | 实时统计当前进行中的BF交易数量，数据来源：订单表筛选状态为进行中 |
| Developer链接 | 可点击跳转至开发者主页，链接地址根据游戏配置 |
| 入口按钮 | 点击跳转至 `/bloxfruits` |

> **注**：BF暂无抽奖功能，因此不展示Giveaways字段；后续支持时可增加。

#### 4.5.3 GAG Top Trading Items（复用现网）

复用现网首页"Hot Trading Items"模块的数据和样式，具体逻辑详见现网实现。

**底部入口**：
- 文案："VIEW XXX MORE GAG ITEMS"，XXX为当前可交易道具总数
- 点击跳转至 `/growagarden/value`

#### 4.5.4 BF Hot Trading Items（新增）

**数据来源**：BF道具数据库，按热度/交易量排序取Top N

**展示样式**：
- 复用GAG热门道具的卡片样式结构
- 道具卡片展示：道具图片、道具名称、稀有度标签

**底部入口**：
- 文案："VIEW XXX MORE BF ITEMS"，XXX为当前可交易道具总数
- 点击跳转至 `/bloxfruits/value`

#### 4.5.5 Ultimate Trading Hub（新增）

**标题**："Your Ultimate Trading Hub"
...
| 游戏 | 展示内容 | 数据来源 |
|------|----------|----------|
| Grow a Garden | `XXX Trades` | 历史合计交易数量（订单表统计已完成订单总数） |
| Blox Fruits | `XXX Trades` | 历史合计交易数量（订单表统计已完成订单总数） |

**CTA按钮**：
- 文案："TRADE NOW!"
- 点击行为：滚动回页面顶部Hero区域（用户可从Hero区选择游戏进入）

#### 4.5.6 Why Choose Tradekitsune（保留）

复用现网"Why Choose Tradekitsune"模块，内容和样式不变。

**展示内容**：
- 平台介绍文案："Tradekitsune is a fan-made website."
- 三大优势卡片：
  - SECURE：Anti-scam Protection
  - FAST：Real-time Market Matching  
  - VALUES：Live Market Trends Tracking
- Worldwide标识

#### 4.5.7 FAQ（保留）

复用现网FAQ模块，内容为平台通用FAQ：

- What's This Site For?
- Why should I login with Roblox and Is it Safe?
- How is Tradekitsune different from other gaming websites?
...
> **设计原则**：游戏首页主体复用现网GAG首页的模块结构和样式，新增"游戏信息区"和"Value Top List"模块，调整底部通用导流位增加跨游戏入口。

#### 4.6.1 模块布局（通用结构）

以下模块布局同时适用于GAG首页和BF首页：

| 模块位置 | 模块名称 | 内容说明 | 是否新增/调整 |
|----------|----------|----------|---------------|
| 1 | 游戏信息区 | 游戏介绍 + 核心数据统计 | ✅ 新增 |
| 2 | Value Top List | 右侧栏展示道具价值排行 | ✅ 新增 |
| 3 | Hot Trading Items | 热门交易道具列表 | 复用现网 |
| 4 | Featured Articles | 精选文章（仅GAG） | 复用现网 |
| 5 | Latest Trading List | 最新订单列表 | 复用现网 |
| 6 | 底部工具入口 | Toolbox工具入口 | 复用现网 |
| 7 | 跨游戏导流位 | 其他游戏入口Banner | ✅ 调整 |
| 8 | Footer | 公共Footer | 见4.2 |
...
| 字段 | GAG示例 | BF示例 | 说明 |
|------|---------|--------|------|
| 游戏图标 | GAG Logo | BF Logo | 游戏官方Logo |
| 游戏名称 | Grow a Garden | Blox Fruits | 游戏全称 |
| 游戏描述 | "Are you ready to grow your very own garden?..." | "Welcome to Blox Fruits!..." | 简短游戏介绍 |
| 适龄标签 | "Maturity: Minimal - Ages 5+" | "Maturity: Mild - Ages 5+ Violence" | Roblox官方分级 |
| Developer | 可点击跳转 | 可点击跳转 | 跳转至开发者主页（配置项） |

**右侧 - 核心运营数据**：

| 数据项 | 数据来源 | 说明 |
|--------|----------|------|
| Giveaways | 统计当前进行中的抽奖活动数量（status=ongoing） | 仅GAG展示，BF暂无 |
| Ongoing Trades | 统计当前进行中的交易订单数量 | 两个游戏都展示 |
| Completed Trades | 历史合计已完成交易数量 | 可选展示 |

**Developer跳转配置**：
- 由管理端配置各游戏的开发者主页链接
- GAG示例：`https://www.roblox.com/groups/xxx`
- BF示例：`https://www.roblox.com/groups/yyy`

#### 4.6.3 Value Top List（新增模块）

**位置**：页面右侧栏，与Hot Trading Items并列

**数据来源**：各游戏的道具Value数据库，与Value列表页使用同一份数据

**展示规则**：
- 取Value值Top N的道具（N根据页面空间确定，建议5-10个）
- 按Value值从高到低排序
...
| 排名序号 | 1、2、3... |
| 道具图片 | 道具缩略图 |
| 道具名称 | 可点击跳转至道具Wiki详情页 |
| Value值 | 当前Value数值 |
| 趋势图 | 迷你趋势图（7天/30天） |

> **注**：右侧栏展示时仅展示到趋势图，不展示Value列表页表格中的其他扩展字段（如demand、age等）。

**底部入口**：
- 文案："View Full Value List"
- 点击跳转至 `/{gameKey}/value`

#### 4.6.4 Hot Trading Items（复用现网）

复用现网"Hot Trading Items"模块的数据查询逻辑和展示样式。

**包含内容**：
- 道具分类Tab（Pets/Crops等根据游戏不同）
- 道具卡片列表
- 工具入口（Token Calculator、Pet Weight Calculator等）
- "VIEW MORE"入口

具体实现详见现网代码。

#### 4.6.5 Featured Articles（复用现网，仅GAG）

复用现网"Featured Articles"模块。

**适用范围**：仅GAG首页展示

**BF说明**：MVP阶段BF暂无文章内容，待内容积累后再添加该模块。

#### 4.6.6 Latest Trading List（复用现网）

复用现网"Latest Trading List"模块。

**数据来源**：各游戏的订单数据，按创建时间倒序
...
**入口按钮**：
- "POST A TRADE"：跳转至 `/{gameKey}/trade/create`
- "VIEW ALL TRADES"：跳转至 `/{gameKey}/tradelist`

#### 4.6.7 底部工具入口（复用现网）

复用现网Toolbox工具入口模块，根据游戏配置展示可用工具。
...
|------|------|
| 游戏图标 | BF Logo |
| 标题 | "Also Trading: Blox Fruits" |
| 描述 | "Explore Blox Fruits trades" |
| CTA按钮 | "Enter Blox Fruits" → 跳转 `/bloxfruits` |
...
|------|------|
| 游戏图标 | GAG Logo |
| 标题 | "Also Trading: Grow a Garden" |
| 描述 | "Explore GAG trades" |
| CTA按钮 | "Enter GAG" → 跳转 `/growagarden` |
...
|--------|-----|
| 游戏名称 | Grow a Garden |
| 游戏描述 | Are you ready to grow your very own garden?... |
| 适龄标签 | Maturity: Minimal - Ages 5+ |
| Developer链接 | 配置项 |
| 道具分类Tab | Pets / Crops |
| Value List数据 | GAG道具Value数据 |
| Featured Articles | ✅ 展示 |
| Giveaways统计 | ✅ 展示 |
...
|--------|-----|
| 游戏名称 | Blox Fruits |
| 游戏描述 | Welcome to Blox Fruits!... |
| 适龄标签 | Maturity: Mild - Ages 5+ Violence (Repeated/Mild) |
| Developer链接 | 配置项 |
| 道具分类Tab | Fruits / Swords / Accessories（待确认） |
| Value List数据 | BF道具Value数据 |
| Featured Articles | ❌ 暂不展示（MVP阶段无内容） |
| Giveaways统计 | ❌ 暂不展示（MVP阶段不支持） |

**注意：BF的MVP阶段暂无文章模块，待内容积累后再添加**
...
#### 4.10.7 FAQ模块
文案待更新（与创建交易页用不同FAQ内容）


#### 4.10.8 SEO配置（待细化，未定稿）
...
**交易信息卡片的区别**：只展示具体这个道具是普通或永久及对应的value和Beli、Robux币值（非KMB，带千分位）；如为Fruit则在道具图标处带上普通/永久的角标（PHYS/PERM）。
