...
| 阈值 | 触发条件 | 处理方式 | 提示位置 |
|------|----------|----------|----------|
| **40%** | 价差 > 40% | **禁止提交** | POST TRADE按钮置灰 + 行内错误提示 |
| **60%** | 价差 > 60% 且 任一方含Gamepass且不含Perm/Limited | **允许提交但显示凑数提示** | 行内警告提示 |

##### 4.9.5.3 40%阈值 - 禁止提交

**触发条件**：Beli Value价差 > 40%

**UI表现**：
- POST TRADE按钮置灰，不可点击
- 按钮下方显示红色错误提示：
```
⚠️ Trade value difference exceeds 40%. Please adjust items to balance the trade.
```

##### 4.9.5.4 60%阈值 - 凑数提示（Gamepass场景）

**触发条件**：
1. Beli Value价差 > 60%
2. 任一方包含Gamepass类道具
3. 任一方**不**包含Perm或Limited类道具

**UI表现**：允许提交，但在交易面板区域显示橙色警告：
```
...
**提示位置**：Beli Value展示区下方，行内展示

#### 4.9.6 特殊道具警告（Perm/Limited）

##### 4.9.6.1 触发条件

交易任一方包含 **Permanent** 或 **Limited** 类型道具时触发
...
**文案**：
```
⚠️ Auto-Redeem Notice

This trade includes Permanent or Limited items. These items will be automatically redeemed to your account upon trade completion and cannot be re-traded.
```
...
| 场景 | 展示内容 |
|------|----------|
| 价差 > 40% | 显示红色警告：`⚠️ Trade value difference exceeds 40%. This trade may fail in-game.` |
| 价差 > 60% 且含Gamepass且不含Perm/Limited | 显示橙色凑数提示（文案同4.9.5.4） |
| 含Perm/Limited | 显示Auto-Redeem Notice（文案同4.9.6.2） |
