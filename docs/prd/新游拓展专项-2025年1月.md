...
| 维度 | GAG（基线） | BF（本次） | 说明 |
|------|-------------|------------|------|
| **页面标题区** | Create a New Trade of {游戏全称} | Create a New Trade of {游戏全称} | 主标题增加游戏名称；副标题不变 |
...
**道具卡片展示**：
- 使用BF道具卡片（展示PERM标签和F/L/G类型角标）
- 格子底色对应道具稀有度（Common/Uncommon/Rare/Legendary/Mythical/Premium）
- Permanent类型以角标形式标识，默认Physical果实不展示
- 添加、删除等基础交互不变
...
- 底部提示信息：简要说明什么是Value和Beli Value（待补充文案）
  

##### 4.9.3.3 Beli Value展示区（新增）

**位置**：中间WFL对比区下方，紧贴原有进度条
...
```
**Beli Value计算与展示逻辑**：
- 拼接逻辑：图标 + Beli Value: {左侧面板道具的Beli合计值（KMB标记）} vs  {左侧面板道具的Beli合计值（KMB标记）} (Diff: {左右侧Beli合计值差值的绝对值} / 价差比例)
ps, 价差比例**详见4.9.5.3 价差计算公式**
- 展示逻辑：场景B中不展示（任一方道具中含Perm果实或Limited）→ 豁免Beli校验
ps, 场景详见**4.9.5 Beli Value校验与提示逻辑（完整流程）**


**Beli Value说明文案**（点击ⓘtooltip后展示）：
```
Beli Value is used in-game to determine if a trade can be completed. Trades with large Beli Value differences (especially involving Physical Fruits or Gamepasses) may fail. See FAQ for details.
```
...
```
优先级1：场景B（左/右含任一Perm果实或Limited）→ 豁免Beli校验
...
**UI表现**：
- POST TRADE按钮置灰，不可点击
- **提示位置**：Beli Value展示区下方，显示红色错误提示：
```
🔴 Warning
Beli value difference is too large! (Max. 40%)
Value must be around the same, otherwise In-game trade will fail. 
```
...
**UI表现**：允许提交，但中间的提示信息区显示特殊物品警告（见4.9.6节）
...
To bypass the 40%-70% value limit in-game, please add Filler Fruits to balance, because Gamepasses count as $0 Beli.
```
**提示位置**：Beli Value展示区下方，行内展示（橙色）
...
交易任一方包含 **Permanent果实**、**Limited** 或 **Gamepass** 类型道具时触发
...
| 创建交易者（卖方） | 对方道具中含Perm/Limited/GP | 无需提示 |
...
| **关键元素** | 有token；有稀有度角标；有数量角标；最后一格有"x more"逻辑 | 无token；稀有度以格子颜色区分，无角标；无数量角标；最后一格无"x more"逻辑；增加"Permanent"类型标签（普通果实不展示，仅永久果实展示）；增加F/L/G（Fruit/Limited/Gamepass）类型角标 | ✅ 已确认 |
