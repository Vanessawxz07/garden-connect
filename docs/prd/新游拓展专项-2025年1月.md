...
```
Beli Value (equal to item's Beli) is used in-game to determine if a trade can be completed. Trades with large Beli Value differences (especially involving Physical Fruits or Gamepasses) may fail. See FAQ for details.
```
