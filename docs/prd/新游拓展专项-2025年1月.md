...
- Base Value is determined by the item's Beli, rarity, type, and whether it's Physical or Permanent.
- Market Adjustment applies real-time data from the trading community to calculate the final value.

Note: Special items like Gamepasses and Limiteds have $0 Beli Value in-game, which may affect trade completion.
```
