...
---
## 🔄 2025年3月更新：公共首页迭代(P0, 需在第三款游戏上线前完成)

### 公共首页
视觉稿：https://www.figma.com/design/UpWKSFehc5CiNRo1GA3sPG/home?node-id=2924-27718&t=JC8kmE2zDqjDdOr2-0

1、**增加新游入口**
卡片平铺展示，大屏端一行3个，小屏端一行2个。

支持后续配置生效，配置项：是否展示、图片、游戏短描述文案、展示顺序。
<img width="1349" height="694" alt="局部截取_20260304_180232" src="https://github.com/user-attachments/assets/657098b2-2c8c-4185-a2ca-c230ac95f1c5" />


2、**末位"Coming Soon"兜底卡片**

当游戏卡片无法填满整行时，在末位空位展示一张"More games coming soon!"占位卡片（图片待提供），避免行尾留白。

<img width="1009" height="623" alt="局部截取_20260309_145218" src="https://github.com/user-attachments/assets/a78fc9e7-34db-43ec-8e0e-5bd7cce064fd" />


**展示规则：**
| 屏幕尺寸 | 每行卡片数 | 展示条件（游戏卡片数量） |
|----------|-----------|------------------------|
| 中小屏（手机/平板） | 2 | 游戏数为奇数（1、3、5…）时展示 |
| 大屏（桌面） | 3 | 游戏数量除以3，余数不为0（即4、5、7…）时展示 |

**卡片内容：**
- 图片：Coming Soon主题配图（与现有游戏卡片等高等宽）
- 标题：**More games coming soon!**
- 描述文案：Vote for your favorite
- 点击交互：整张卡片可点击，跳转 Discord 投票入口（新窗口打开，链接：https://discord.gg/88nnmG4dvD）

**与原"底部提示文案"的关系：** 原 4.5.2 Hero区底部的 "More games coming soon! Vote for your favorite" 文字链内容迁移至此卡片中，底部文案简化为：
```
We started with Grow a Garden, now we're growing with you.
Learn our story
```
仅保留"Learn our story"文字链（跳转 about us 页面）。


**3/11新增-（P1，可线上迭代）**：游戏卡片标题前增加新游标识，根据配置展示。
<img width="737" height="740" alt="局部截取_20260311_111532" src="https://github.com/user-attachments/assets/6ebc504a-8dc5-41d0-be24-30613da6ccac" />


3、**增加新游热门道具推荐模块**

支持后续配置生效，配置项：是否展示、标题文案、展示顺序
<img width="1540" height="674" alt="局部截取_20260304_180429" src="https://github.com/user-attachments/assets/c30c4e8b-f0a3-48cc-9f5d-7e22b594c74f" />


---

### 聊天页
视觉稿：https://www.figma.com/design/oS9Yg9snhnkrTR3QF8f0Bf/chat?node-id=3219-7524&p=f&t=l3QG9je4G4lLwVdw-0
随着新游上线，聊天页分游戏筛选Tab的横向空间不足，改为**单一下拉选择器**（一次性改造）。
...
| 游戏选项 | 各游戏图标 + 名称 + 各自未读数 |
| 默认选中 | 纯公共页面进入"All Message"；强游戏属性的场景则按照游戏上下文的逻辑带入参数，进入聊天时自动选中对应游戏（参考导航栏动态切换逻辑中的游戏参数带入） |
| 空状态 | 按选中的筛选值展示对应游戏聊天的空状态和CTA |

<img width="1115" height="424" alt="局部截取_20260309_120738" src="https://github.com/user-attachments/assets/64ae6541-fe96-41dd-9f28-028f989b7758" />



---

## 🔄 2025年3月更新：导航栏中性化改造+双导航栏动态切换逻辑（P1）

> **更新日期：** 2025-03-04
> 视觉稿：https://www.figma.com/design/4mDwVh4lJ2t1Yy7EcINY5t/UI-Kit?node-id=5347-868&p=f&t=QSfpGQzhvTDN5fWo-0
> 

> **更新背景：** 在实际体验中发现，用户在Giveaway、聊天页等强游戏属性的公共页面中，如果导航栏没有对应游戏上下文，容易误以为是 bug。此前仅做了零散的小优化（根据页面内游戏 tab 选择对应URL参数，导航栏据此展示对应游戏导航栏），本次结合导航栏整体迭代进行全面规划。
...
| 元素 | 说明 |
|------|------|
| **TradeKitsune Logo** | 平台 Logo，点击回到 `/` |
| **All Games ▼** | 游戏选择器，hover或点击则下拉展示所有可用游戏，点击跳转对应游戏首页 |
| **Community ▼** | 新增一级入口，下拉菜单含：Discord（由Contributors页面改造）、YouTube、Twitter/X（https://x.com/tradekitsune）、Feedback（tally表单：https://tally.so/r/wz6Kb0，表单中增加展示support邮箱地址）、about us |
| **Help ▼** | 新增一级入口，下拉菜单含：Beginner's Guide、Safe Trading Guide、Updatelog、FAQ |
| **💬 MESSAGE** | 消息入口，逻辑不变，同 4.1.4 |
| **👤 用户头像/Login** | 用户区，逻辑不变，同 4.1.4 |

**Contributors页面改造**：顶部微调，展示分游戏不同的discord入口（支持新游上线后配置添加）。
<img width="1430" height="637" alt="局部截取_20260304_175200" src="https://github.com/user-attachments/assets/50be5a18-4ab2-4f31-9d25-dc1a58ac8566" />


#### 与历史方案的差异
| 维度 | 原 MVP 方案一（GAG优先模式） | 本次更新（中性化公共导航栏） |
|------|---------------------------|---------------------------|
| 功能菜单 | 默认展示 GAG 菜单入口 | 不展示任何游戏功能菜单 |
| 游戏选择器 | 下拉切换，默认 GAG | 中性 "All Games" 状态 |
| 适用阶段 | 仅阶段一 | 全阶段适用，无需大改 |

### 特殊关注

- **Giveaways不在公共导航栏展示** 因为 Giveaways 始终有游戏上下文，默认 GAG tab，不存在无游戏选择的空状态，仅在游戏导航栏中展示，点击进入giveaways的对应tab
- **Discord入口仅在游戏导航栏中出现**：跳转对应游戏的discord频道，公共导航栏中则不展示。各端统一。
- **公共导航栏增加公共首页入口**
- **游戏导航栏增加游戏首页入口**
...
> **与历史版本的关系：** 本次更新替代 4.1.3 中的"MVP方案一（GAG优先模式）"，不影响 4.1.1、4.1.2、4.1.4、4.1.5 等已定稿内容。

### 方案演进过程
初期考虑了三个方案：
- **方案A**：公共导航栏 + 游戏上下文指示器增强（Game Selector 视觉增强）
- **方案C**：公共导航栏 + 游戏快捷入口条（Sub-Bar）
- **方案B**：动态切换导航栏

最终确认采用双导航栏 + 动态切换，将页面分为三类进行分别处理：
- 游戏页面：在游戏路径下，明确分属不同游戏，以及**特例处理（Giveaway详情页也属此类）**--直接使用游戏导航栏（逻辑不变）
- 纯公共页面：如（ /about-us、/FAQ等）--采用中性化改造后的公共导航栏（逻辑不变）
- 强游戏属性的公共页面：路径上不属于任何游戏，整站共用，但强游戏属性，url带游戏参数：如 /giveaways 、/user、/chat--采用**动态切换**，无参数时使用公共导航栏；有参数时（即有游戏上下文），使用对应游戏导航栏；用户在页面内切换游戏 tab 时，URL 参数同步更新，导航栏跟随变化。


#### 参数来源逻辑
游戏上下文参数的产生方式：
1. **路径自带**：`/{gameKey}/*` 页面路径本身确定游戏
2. **页面内 tab 切换**：如 Giveaway、chat、user页面切换游戏 tab → URL 参数同步更新
3. **带游戏参数的公共页面链接**：如用户分享的 `/giveaways?game=bf` 链接
4. **从游戏页面进入**：如从 `/growagarden/tradelist` 或GAG的giveaway详情页 点击用户头像 → `/user/{uid}?game=gag`
5. **从游戏属性页面进入**：如从已带有游戏参数/giveaways 、/user、/chat页面和Giveaway详情页进入


补充：游戏导航栏更新所有游戏默认的二级入口顺序
导航栏一级入口文案和顺序：一级Trade 
→ 二级 "Trade Listings"文案改为"Find Trades"放首位，"Post a Trade"放第2位，第3位为"Treadable Items"。
---

## 🔄 2025年3月更新：footer中性化改造（P1）
GAG与BF的游戏内页入口取消，替换为"平台支持游戏"。

标题：Supported Games
入口：{游戏名称} (点击跳转对应游戏首页，每新上一款游戏，则此处自动添加)

<img width="1488" height="379" alt="局部截取_20260304_175745" src="https://github.com/user-attachments/assets/ce3e9c09-8610-4850-9d3a-bb369a428adc" />

品牌简介文案更新规则：
Your trusted Roblox games trading hub.
Trade smarter with a secure marketplace for Grow a Garden, Blox Fruits, and {数字=游戏数量-2；如=1则不展示} more roblox games.




---
> ⬇️ 以下为历史版本内容（2025年1月原始版本），保持不变供回顾对比。
...
We started with Grow a Garden, now we're growing with you. Learn our story

交互："Learn our story"（文字链，跳转网站about us页面）。

> **3月更新：** 原"More games coming soon! Vote for your favorite"文字链已迁移至游戏卡片区的末位"Coming Soon"兜底卡片中，详见"3月更新-公共首页-第3点"。
