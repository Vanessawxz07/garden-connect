...
**顶部网站标题**：
```
主标题（H1）：Trade Roblox Game Items Safe & Fast - GAG · BF
副标题（H1）：The trusted marketplace for GAG, Blox Fruits & more
```
**运营数据**：
- Ongoing Trades：与现网字段口径不变，但不区分游戏，使用整站合计。
...
| 入口按钮 | "START TRADING" |
交互：点击图标、游戏标题、按钮均跳转至各游戏首页

#### 4.5.3 GAG Hot Trading Items（复用现网）
- 标题行支持点击，点击跳转至 `/growagarden/trade`
复用现网首页"Hot Trading Items"模块的数据和基础样式，但只展示一行，交互改为横滑显示更多，最多拉取12个道具。

**底部入口**：
- 按钮1："VIEW MORE GAG ITEMS"，点击跳转至 `/growagarden/trade`
- 按钮2："VIEW GAG TRADES"，点击跳转至 `/growagarden/tradelist`


#### 4.5.4 GAG Value List
- 标题："Pet Value List of Grow a Garden - {日期，月日年}"，支持点击跳转到`/growagarden/value`
- 列表：直接复用value列表页中的列表模块，但只展示top5的道具


#### 4.5.5 BF Hot Trading Items（复用BF组件）
- 复用BF道具列表页的卡片样式结构，交互和拉取数量与GAG相同（只展示一行，交互改为横滑显示更多，最多拉取12个道具）。
- 标题行支持点击，点击跳转至 `/bloxfruits/trade`
**数据来源**：BF道具数据库，排序逻辑也和GAG相同。

**底部入口**：
- 按钮1："VIEW MORE BF ITEMS"；点击跳转至 `/bloxfruits/trade`
- 按钮2："VIEW BF TRADES"；点击跳转至 `/bloxfruits/tradelist`


#### 4.5.6 平台介绍 - What We Offer（新增）

**标题**："Your Ultimate Roblox Trading Hub"
**副标题**："What We Offer"
...
| 📈 | Live Values | Real-time price trends updated daily. |
| 🎁 | Weekly Giveaways | Win free items—join or host your own! |
| ✍️ | Fresh Content | Guides, tips & videos from our creators. |
| 💬 | Active Discord | Chat with 10K+ traders & get instant help. |


#### 4.5.7 价值点 - Why Choose Tradekitsune（复用）
模块标题："🏆Why Choose Tradekitsune"

**展示内容**：(待优化，根据about us页面内容提炼)
- 平台介绍文案："Tradekitsune is a fan-made website."
- 品牌图
- 四大优势
- 分享按钮、TRUSTPILOT按钮：复用giveaway聚合页的按钮逻辑、样式


#### 4.5.8 FAQ（复用）
FAQ模块每条问题默认折叠，内容为平台通用FAQ：
...
| 游戏描述 | "Are you ready to grow your very own garden?..." | "Welcome to Blox Fruits!..." | 简短游戏介绍，最多展示4行，超长省略 |
| 适龄标签 | "Maturity: Minimal - Ages 5+" | "Maturity: Mild - Ages 5+ Violence" | Roblox官方分级 |
| Developer | 开发者名称+开发者Roblox主页url | 可点击跳转 | 跳转至开发者主页 |

**核心数据**：
| 数据项 | 数据来源 | 说明 |
|--------|----------|------|
| Giveaways | 展示抽奖活动数量（与现网首页的拉取口径一致） | 仅GAG展示(点击后跳转`/growagarden/giveaways`)，BF暂无 |
| Ongoing Trades | 统计当前进行中的交易订单数量 | 两个游戏都展示，点击后跳转`/growagarden/tradelist` |
...
#### 4.6.6 Hot Trading Items（复用现网）
- 模块复用现网"Hot Trading Items"模块的数据查询逻辑和展示样式，展示组件则使用各游戏对应的
- 标题文案更新为："Hot Trading Items of {游戏全称}"


#### 4.6.7 Latest Trading List（复用现网）
- 复用现网"Latest Trading List"模块的数据查询逻辑和展示样式，展示组件则使用各游戏对应的
- 标题文案不变
...
样式与交互复用现网逻辑，但需要区分拉取同一youtube频道中不同游戏的视频内容。
- 方案1：按频道中的"播放列表"拉取
...
- 模块标题：
- 描述：
- 游戏卡片入口：游戏图标+游戏名称+道具数量+订单数量（所有创建的交易数量，KMB标记）+"TRADE NOW!"按钮；卡片整体响应点击，跳转`/对应游戏/tradelist`页面；更多游戏"More Games Coming Soon..."
- 底部用户数据：静态文案
