...
| **Open Offer NLF区域** | 右侧6格 | 右侧**12格**（6x2） | NLF区域保持12格，方便设置不想要的道具 |
...
| **特殊道具警告** | 无 | **Perm/Limited自动赎回警告** | 行内提示，非弹窗 |
| **Trade Summary格式** | `{名称} x{数量} · {稀有度} · {Age} · {kg} · Value {值}` | `{名称} x{数量} · {Category} · Value {值} · Beli Value {值}` | 字段调整 |
| **底部工具入口** | GAG Pet Value、GAG Value Calculator | Trade Value Calculator、Blox Fruits Wiki、Blox Fruits Tier List | 3个入口 |
| **FAQ** | GAG相关内容 | BF相关内容（待补充） | 后续提供 |

#### 4.9.2 页面结构（复用不变）

以下模块逻辑与GAG完全一致，不再赘述：
- 页面标题区（Create a New Trade / Choose what you're offering and what you want.）
- Tab切换（Specific Trade / Open to Offers）
- 中间Value对比区（WIN/LOSS/FAIR状态、进度条）
- 操作按钮（CLEAR ALL / POST TRADE）
- How to Post a Trade教程（4步骤，文案复用，截图需替换为BF）
- Friendly Reminders（5条提示，文案复用）
...
**槽位调整**：
- 左侧 My Items：**4格**（2x2网格），显示 `X/4`
- 右侧 Looking for Items：**4格**（2x2网格），显示 `X/4`

**道具卡片展示**：
- 使用BF道具交易信息卡片组件（见5.4节）
- 格子边框颜色对应稀有度（Common/Uncommon/Rare/Legendary/Mythical）
- Physical/Permanent类型以角标形式标识

##### 4.9.3.2 Trade Summary格式

```
Trade Summary                    Value Details
- Dragon x1 · Fruit · Value 2.5M · Beli Value 3.5M
- Dark Blade x1 · Limited · Value 1.2M · Beli Value 0

Total Value: 3,700,000
Total Beli Value: 3,500,000
```

**拼接规则**：`{道具名称} x {数量} · {Category} · Value {Value值} · Beli Value {Beli Value值}`

##### 4.9.3.3 Beli Value展示区（新增）

**位置**：中间Value对比区下方，紧贴原有进度条
...
│  ████████████████░░░░  Offering Value: 3.7M           │
│  ████████████░░░░░░░░  Looking for Value: 2.8M        │
├────────────────────────────────────────────────────────┤
│  📊 Beli Value: 3.5M vs 2.8M (Diff: +700K / +25%)     │  ← 新增行
│  ⓘ Beli Value is used in-game to determine trade...  │  ← 新增说明
└────────────────────────────────────────────────────────┘
```

**Beli Value说明文案**（点击ⓘ展开或常驻小字）：
```
Beli Value is used in-game to determine if a trade can be completed. Trades with large Beli Value differences (especially involving Gamepasses) may fail. See FAQ for details.
```
- "FAQ"为文字链，跳转页面底部FAQ区域
...
**Value Requirement滑块**：
- 移除Token相关，简化为纯Value模式
- 显示：`Lowest Value I'll accept: {value值(KMB标记)}`
- 显示：`Max Value: {value值(KMB标记)}`
...
#### 4.9.5 Beli Value校验与提示逻辑（完整流程）

##### 4.9.5.1 场景判断优先级

交易校验按以下**优先级顺序**判断，命中即停止：

```
优先级1：场景B（含Perm/Limited）→ 豁免Beli校验
优先级2：场景C（含Gamepass但不含Perm/Limited）→ 60%阈值校验
优先级3：场景A（纯物理 vs 纯物理）→ 40%阈值校验
```
...
flowchart TD
    ...
    ALLOW_C1 --> WARN_C1[仅显示特殊物品警告]
    CALC_60 -->|> 60%| ALLOW_C2[✅ 允许提交]
    ALLOW_C2 --> WARN_C2[显示特殊物品警告 + 凑数提示]
...
| 场景 | 道具组合 | 核心判断逻辑 | 能否提交 | UI反馈 |
|------|----------|--------------|----------|--------|
| **A** | 纯物理 vs 纯物理 | 价差 > 40% | ❌ 阻止 | 红色错误提示 + 按钮置灰 |
| **B** | 任一方含Perm/Limited（不管是否有GP） | 直接豁免（不做Beli校验） | ✅ 允许 | 特殊物品警告 |
| **C** | 任一方含Gamepass且不含Perm/Limited | 价差 > 60% 时增加凑数提示 | ✅ 允许 | 特殊物品警告 + (可选)凑数提示 |

##### 4.9.5.5 场景A - 纯物理交易 - 40%阈值

**触发条件**：
- 双方道具均为普通物理道具（Fruit等）
- 不含任何Perm/Limited或Gamepass

**校验规则**：Beli Value价差 > 40% → 阻止提交
...
Beli value difference: {实际价差}% (Max. 40%) - Value must be around the same. Otherwise In-game trade will fail.
```

##### 4.9.5.6 场景B - 含Perm/Limited交易 - 豁免校验

**触发条件**：交易任一方包含 **Permanent** 或 **Limited** 类型道具（无论是否同时含有Gamepass）

**校验规则**：直接豁免Beli Value校验，允许任意价差

**UI表现**：允许提交，但显示特殊物品警告（见4.9.6节）

##### 4.9.5.7 场景C - 含Gamepass不含Perm/Limited - 60%阈值

**触发条件**：
- 任一方包含Gamepass类道具
- 双方均**不**包含Perm或Limited类道具

**校验规则**：
- 价差 ≤ 60%：允许提交，仅显示特殊物品警告
- 价差 > 60%：允许提交，显示特殊物品警告 + 凑数提示

**UI表现 - 凑数提示**（仅价差 > 60% 时显示）：
```
⚠️ Beli Value Warning

To bypass the 40%-70% value limit in-game, please add Filler Fruits to balance, as Gamepasses count as $0 Beli.
```

**提示位置**：Beli Value展示区下方，行内展示（橙色样式）

#### 4.9.6 特殊物品警告（场景B/C共用）

##### 4.9.6.1 触发条件

交易任一方包含 **Permanent**、**Limited** 或 **Gamepass** 类型道具时触发

##### 4.9.6.2 警告文案区分规则

根据**道具所在方**与**用户角色**区分不同文案：

| 用户角色 | 道具所在 | 应看到的警告 |
|----------|----------|--------------|
| 创建交易者（卖方） | 自己的道具中含Perm/Limited/GP | Trade Reminder |
| 创建交易者（卖方） | 对方道具中含Perm/Limited/GP | Auto-Redeem Alert |
| Make Offer者（买方） | 自己报价中含Perm/Limited/GP | Trade Reminder |
| Make Offer者（买方） | 卖家道具中含Perm/Limited/GP | Auto-Redeem Alert |

**简化规则**：
- **你提供的特殊道具** → 显示 Trade Reminder（提醒对方购买方式）
- **你将接收的特殊道具** → 显示 Auto-Redeem Alert（提醒自己会自动激活）

##### 4.9.6.3 警告文案

**Trade Reminder（道具提供方应看到）**：
```
⚠️ Trade Reminder

To trade this item, you MUST purchase it via "Gift" → "Store in Inventory". Items bought via "Buy" are auto-equipped and CANNOT be traded.
```

**Auto-Redeem Alert（道具接收方应看到）**：
```
⚠️ Auto-Redeem Alert

The special item will auto-activate upon receipt. You cannot trade it again.
```

##### 4.9.6.4 展示方式

**方案**：行内提示（非弹窗），在Beli Value展示区下方显示

##### 4.9.6.5 展示逻辑示例

| 场景 | 创建交易者看到 | 接单者/报价者看到 |
|------|----------------|-------------------|
| 左侧（我的道具）含Perm/Limited/GP | Trade Reminder | Auto-Redeem Alert |
| 右侧（想要的道具）含Perm/Limited/GP | Auto-Redeem Alert | Trade Reminder |
| 双方都含特殊道具 | Trade Reminder + Auto-Redeem Alert | Trade Reminder + Auto-Redeem Alert |
| 双方都不含 | 无 | 无 |

#### 4.9.7 Value Details弹窗

**触发**：点击Trade Summary区域的"Value Details"链接

**弹窗内容调整**：
- 移除Token公式相关内容
- 展示各道具的Value和Beli Value计算明细
- 底部增加Beli Value说明
...
[Dragon] Value 2,500,000 · Beli Value 3,500,000
[Dark Blade] Value 1,200,000 · Beli Value 0 (Gamepass)

─────────────────────────────────────
Total Value: 3,700,000
Total Beli Value: 3,500,000

ⓘ Gamepasses have $0 Beli Value in-game trading.
```
...
图标：Wiki图标
标题：Blox Fruits Wiki
描述：Explore all fruits, swords, and gamepasses
跳转：/bloxfruits/wiki
```
...
**预留结构**：
```
Q: How does Beli Value affect my trade?
A: [待补充]

Q: Why can't I submit my trade?
A: [待补充]

Q: What are Filler Fruits and why do I need them?
A: [待补充]

Q: What happens when I trade Permanent or Limited items?
A: [待补充]
```
...
##### 4.10.3.2 Beli Value展示区

**位置**：中间Value对比区，紧贴进度条下方
...
**说明**：Specific Trade订单在创建时已通过Beli Value校验，不可能存在场景A（纯物理价差>40%）的情况。因此订单详情页仅需展示特殊物品警告。

**警告展示规则**：
- 根据订单中的道具类型，展示对应的特殊物品警告（Trade Reminder / Auto-Redeem Alert）
- 警告文案按提供方/接收方区分（详见4.9.6节）
- 若订单含Gamepass且价差>60%，同时展示凑数提示

| 订单道具情况 | 买方（接单者）看到 |
|--------------|-------------------|
| 卖家道具含Perm/Limited/GP | Auto-Redeem Alert |
| 想要的道具含Perm/Limited/GP | Trade Reminder |
| 含GP不含Perm/Limited，价差>60% | 特殊物品警告 + 凑数提示 |
...
**底部信息**：
- `Lowest Value required: {value值(KMB标记)}`（移除Token）
- Not Looking for (NLF)：显示卖家不想要的道具
...
**"How is value calculated?"点击弹窗**：
```
How is value calculated?

Value consists of two parts: Base Value and Market Adjustment.
- Base Value is determined by the item's rarity, type, and whether it's Physical or Permanent.
- Market Adjustment applies real-time data from the trading community to calculate the final value.

Note: Gamepasses have $0 Beli Value in-game, which may affect trade completion.
```

##### 4.10.4.2 Make Offer时的Beli Value校验

**核心说明**：买方填入报价道具后，需实时计算与卖家道具的Beli Value价差，适用完整的场景A/B/C校验逻辑（与创建交易页4.9.5节一致）。

**校验规则**：

| 场景 | 校验结果 | UI表现 |
|------|----------|--------|
| 场景A：纯物理，价差>40% | ❌ 阻止提交 | MAKE OFFER按钮置灰 + 红色错误提示 |
| 场景B：含Perm/Limited | ✅ 允许提交 | 特殊物品警告 |
| 场景C：含GP不含Perm/Limited，价差≤60% | ✅ 允许提交 | 特殊物品警告 |
| 场景C：含GP不含Perm/Limited，价差>60% | ✅ 允许提交 | 特殊物品警告 + 凑数提示 |

**警告文案规则**：
- 买方报价中的特殊道具 → 显示 Trade Reminder
- 卖家道具中的特殊道具 → 显示 Auto-Redeem Alert
...
**Beli Value展示**：与Specific Trade一致

#### 4.10.5 Offer List模块

**卡片展示**：
- 使用BF交易信息卡片组件（4格布局）
- 点击"more"弹窗中Trade Summary使用BF格式

#### 4.10.6 底部工具入口

与创建交易页4.9.8节一致，3个工具卡片

#### 4.10.7 FAQ模块

**状态**：内容待运营提供（与创建交易页共用FAQ内容）

#### 4.10.8 SEO配置

| 项目 | 内容 |
|------|------|
| **Title** | `Trade Details: [道具名称] - Blox Fruits Trading \| TradeKitsune` |
| **Description** | `View trade details for [道具名称] in Blox Fruits. Check value comparison, Beli Value, and make your offer safely.` |
| **H1** | `Trade Details` |
| **URL** | `/bloxfruits/trade/{orderId}` |
