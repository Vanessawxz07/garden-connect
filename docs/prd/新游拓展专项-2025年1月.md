...
| Hero Banner | 副标题文案 | `Explore The Grow A Garden Trade List And Start GAG Trading Safely Today.` | `Browse the hottes trades from the Blox Fruits community. Trade safely and fast!` |
...
| 工具文字链入口 | 工具1 | `Pet Value Trends` → Value 列表页 | `Blox Fruits Wiki` → Wiki 列表页 |
| 工具文字链入口 | 工具2 | `Pet Value Calculator` → 计算器页 | `Trade Calculator` → trade value计算器页 |
