...
采用**主索引sitemap + 子sitemap**的分层架构，便于管理和扩展。

**Sitemap层级结构：**

| Sitemap文件 | 类型 | 包含页面 |
|------------|------|----------|
| `/sitemap.xml` | 索引sitemap | 引用所有子sitemap（不直接包含URL） |
| `/sitemap-common.xml` | 公共页面sitemap | `/`、用户相关、政策页面等 |
| `/sitemap-gag.xml` | GAG游戏sitemap | GAG首页、交易列表、订单详情、Wiki等 |
| `/sitemap-bf.xml` | BF游戏sitemap | BF首页、交易列表、订单详情、Wiki等 |

**公共页面Sitemap (`/sitemap-common.xml`) 包含：**

| 页面类型 | URL示例 | 更新频率 |
|----------|---------|----------|
|  | `/` | daily |
| 聊天页 | `/chat` | daily |
| 抽奖列表页 | `/giveaways` | daily |
| 抽奖详情页 | `/giveaways/{title}` | weekly |
| 用户主页 | `/user/{uid}` | weekly |
| 新手指引 | `/beginners-guide` | monthly |
| 安全交易指南 | `/safe-trading-guide` | monthly |
| 更新日志 | `/updatelog` | weekly |
| FAQ | `/faq` | monthly |
| 贡献者页 | `/contributors` | monthly |
| 关于我们 | `/about-us` | monthly |
| 隐私政策 | `/privacy-policy` | yearly |
| 用户协议 | `/user-agreement` | yearly |
| Cookie政策 | `/cookie-policy` | yearly |

**游戏页面Sitemap (`/sitemap-{gameKey}.xml`) 包含：**

| 页面类型 | URL模板 | 更新频率 |
|----------|---------|----------|
| 游戏首页 | `/{gameKey}` | daily |
| 交易列表页 | `/{gameKey}/tradelist` | hourly |
| 订单详情页 | `/{gameKey}/tradelist/{orderId}` | daily |
| 创建交易页 | `/{gameKey}/trade/create` | monthly |
| 道具Value列表 | `/{gameKey}/value` | daily |
| Wiki列表 | `/{gameKey}/wiki` | daily |
| Wiki详情页 | `/{gameKey}/wiki/{itemSlug}` | weekly |
| 价值计算器 | `/{gameKey}/tradecalculator` | monthly |
| Tier List | `/{gameKey}/{type}-tierlist` | weekly |
| 文章列表 | `/{gameKey}/articles` | daily |
| 文章详情 | `/{gameKey}/articles/{slug}` | weekly |

**主索引Sitemap (`/sitemap.xml`) 结构：**

```xml
<?xml version="1.0" encoding="UTF-8"?>
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <sitemap>
    <loc>https://域名/sitemap-common.xml</loc>
    <lastmod>2025-01-22</lastmod>
  </sitemap>
  <sitemap>
    <loc>https://域名/sitemap-gag.xml</loc>
    <lastmod>2025-01-22</lastmod>
  </sitemap>
  <sitemap>
    <loc>https://域名/sitemap-bf.xml</loc>
    <lastmod>2025-01-22</lastmod>
  </sitemap>
</sitemapindex>
```
...
**阶段二扩展：**
- 新增游戏时，添加对应 `/sitemap-{newGameKey}.xml`
- 在主索引sitemap中引用新子sitemap
- 公共页面sitemap保持不变
...
| 阶段 | 策略 |
|------|------|
| 阶段一 | 双向内链：GAG模块 ↔ `/growagarden`，BF模块 ↔ `/bloxfruits` |
...
| 3 |  | P0(MVP) | 新增 | ❌ 否 | 否 | — | — | — | 需要改版，覆盖多款游戏的介绍和入口 |
...
We started with Grow a Garden, now we're growing with you. <br/>
More games coming soon! Vote for your favorite | Learn our story

交互："Vote for your favorite"（文字链，在新页面打开discord投票入口，使用Bf discord链接：https://discord.gg/88nnmG4dvD）；"Learn our story"（文字链，跳转网站about us页面）。2个文字链入口并列

<img width="1420" height="444" alt="局部截取_20260202_175500" src="https://github.com/user-attachments/assets/ba2b33cb-f17c-4eaa-b9b5-947bd2448624" />
...
#### 4.7.7 消息列表空状态 - 1/30新增逻辑
当前某tab没有聊天信息的时候，左侧消息列表是空白，容易造成用户误解，因此增加一个兜底的空状态。
视觉稿：https://www.figma.com/design/oS9Yg9snhnkrTR3QF8f0Bf/chat?node-id=3217-4140&t=h17F4vTODubZq1RL-1

- **标题文案**: "No messages yet"
- **副标题文案**: "Start trading to begin a conversation."
- **按钮**："Explore"；点击按钮在原页面跳转对应的游戏首页。

<img width="704" height="1144" alt="企业微信截图_6abb7ea6-132b-4139-8c10-51e7acd9bcf0" src="https://github.com/user-attachments/assets/5d41c42a-9138-44e4-8c16-1a1cb1c7b494" />
...
1/29补充细节：
1）主态是否展示提示信息区————现在实现是展示了的，保持即可，无需修改
2）BF的open订单没有在中间的最出低价提示了（移到右侧面板底部value的问号），中间区域留给提示信息
3）open offer在客态用户出价前，提示信息区可展示特殊物品提示；当右侧面板添加道具后，中间区域则正常进行价差判断和信息提示（参考指定订单）
...
1/30补充：搜索、高级筛选基础功能逻辑复用，文案应按游戏区分，GAG保持pet，BF使用item，其他游戏根据配置。
---------
