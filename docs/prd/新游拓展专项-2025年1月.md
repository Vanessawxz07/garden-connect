...
##### 4.9.5.4 提示信息区展示规则（完整对照表）

以下对照表覆盖**创建交易页**和**订单详情页**的所有场景：
...
| **创建交易页** | Specific Trade | 创建者 | B（含Perm/Limited） | ❌ 不显示 | ⚠️ 特殊物品提示 | 可点击 |
...
| **订单详情页** | Specific Trade | 接单者 | B（含Perm/Limited） | ❌ 不显示 | ⚠️ 特殊物品提示 | 可点击 |
| **订单详情页** | Specific Trade | 接单者 | C（含GP，价差≤60%） | ✅ 显示 | ⚠️ 特殊物品提示 | 可点击 |
| **订单详情页** | Specific Trade | 接单者 | C（含GP，价差>60%） | ✅ 显示 | ⚠️ 特殊物品提示 + 凑数提示 | 可点击 |
| **订单详情页** | Open to Offers | 报价者（Make Offer） | A（价差≤40%） | ✅ 显示 | 无 | 可点击 |
| **订单详情页** | Open to Offers | 报价者（Make Offer） | A（价差>40%） | ✅ 显示 | 🔴 红色错误提示 | ❌ 置灰 |
| **订单详情页** | Open to Offers | 报价者（Make Offer） | B（含Perm/Limited） | ❌ 不显示 | ⚠️ 特殊物品提示 | 可点击 |
| **订单详情页** | Open to Offers | 报价者（Make Offer） | C（含GP，价差≤60%） | ✅ 显示 | ⚠️ 特殊物品提示 | 可点击 |
| **订单详情页** | Open to Offers | 报价者（Make Offer） | C（含GP，价差>60%） | ✅ 显示 | ⚠️ 特殊物品提示 + 凑数提示 | 可点击 |
| **订单详情页** | Open to Offers | 订单主态 | 任意 | 仅展示我的道具信息 | 与创建时一致 | N/A |
...
```
- 场景B时不展示此行
- 点击ⓘ展示Beli Value说明tooltip

**2. 场景A阻止提示（红色错误框）**
```
┌─────────────────────────────────────────────────────────────┐
│ ⛔ Trade cannot be submitted. Beli Value difference exceeds │
│    40%, which will cause in-game trade failure.             │
│    Please adjust your items.                                │
└─────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ ⚠️ This trade includes special items. [View Trade Guidelines]│
└─────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ ⚠️ Beli Value gap is over 60%. Consider adding filler items │
│    to help balance the trade.                               │
│    This trade includes special items. [View Trade Guidelines]│
└─────────────────────────────────────────────────────────────┘
```
...
**弹窗标题**：`Special Item Trade Guidelines`
...
│  ───────────────────────────────────────────────────────    │
│  Permanent Fruits, Limited items, and Gamepasses can only   │
│  be traded through the in-game trade menu, NOT the trade    │
│  hub. Make sure to meet your partner in-game to complete    │
│  the trade.                                                  │
│                                                              │
│  📥 If You're RECEIVING a Special Item                      │
│  ───────────────────────────────────────────────────────    │
│  Permanent Fruits will auto-activate upon receipt and       │
│  cannot be traded again. Limited items and Gamepasses       │
│  will be added to your inventory directly.                  │
│                                                              │
│                                               [ Got it ]     │
└─────────────────────────────────────────────────────────────┘
```
...
**设计说明**：
- 弹窗内容不区分用户角色（创建者/接单者/报价者），统一展示双向提示
- 用户可根据自身情况关注相应内容
- 简化研发逻辑，所有场景复用同一弹窗组件
...
**展示规则**：参照4.9.5节"提示信息区展示规则（完整对照表）"中"订单详情页 - Specific Trade - 接单者"对应的行。
...
**展示规则**：参照4.9.5节"提示信息区展示规则（完整对照表）"中"订单详情页 - Open to Offers - 报价者（Make Offer）"对应的行。
