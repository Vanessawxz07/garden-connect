...
- **权限区别**：部分用户标签对应不同用户类型，即享有不同功能的权限，根据每次需求具体明确。VIP/管理员用户即对应用户的类型，活跃用户为普通用户。
   本期需支持-giveaway活动需要设定发起权限，VIP/管理员用户自动享有该权限，活跃用户默认没有，可通过discord频道向运营申请单独开通。（后续-动态中发post功能也需权限，VIP/管理员用户自动享有该权限）
