...
> **说明**：网站用户体系与运营管理后台是完全独立的两套账号系统。运营管理员使用公司内部账号体系登录管理后台，对网站用户进行管理操作（如授予VIP、添加标签等）。
...
用户标签用于**展示身份**，不控制权限。由运营在管理后台为指定用户添加。

| 标签 | 英文名称 | 颜色 | 说明 |
|------|----------|------|------|
| 平台官方 | **Official** | 金色（#FFD700） | 平台创始团队成员 |
| 内容创作者 | **Creator** | 紫色（#9B59B6） | KOL、社区活跃内容创作者 |
| 游戏开发者 | **Game Developer** | 绿色（#27AE60） | Roblox游戏开发者 |
| 资深用户 | **Veteran** | 蓝色（#3498DB） | 长期活跃的老用户 |
| 信誉交易者 | **Trusted Trader** | 青色（#1ABC9C） | 交易信誉良好的用户 |
...
细粒度的功能权限控制，与用户角色配合使用。

| 权限 | 说明 | VIP默认 | 普通用户默认 |
|------|------|---------|--------------|
| **create_giveaway** | 创建抽奖活动 | ✅ 有 | ❌ 无 |
| **create_post** | 发布动态 | ✅ 有 | ❌ 无 |

**权限优先级**：`手动设置 > 角色默认`
- 普通用户可通过申请获得单独权限（如通过Discord频道申请）
...
### 3.2 访客态（Visitor View）
- **访问权限**：所有用户（根据隐私设置控制可见内容）
- **主要操作**：查看公开信息、关注/取消关注用户
- **主按钮**：「关注」/「已关注」按钮（右上角）

### 3.3 自己的访客态（Self-Visitor View）
