...
**展示规则**：
- **位置**：基础的身份标签(Official/influencer/game developer)默认展示在第一位。位置顺序优先级Official=Influencer=Game Developer > 资深用户=信誉交易者 > 粉丝铭牌（抽奖功能需求中将新增的标签，用户参与抽奖自动获得）；优先级相同就按字母ABC顺序排序
- **个人主页**：最多显示8个标签（超出不展示）
...
#### 4.2.1 头像
默认头像，仅展示，不可交互
12/15更新：站内有用户头像的地方（如交易列表、聊天窗口等）默认交互均为可点击前往该用户主页。
