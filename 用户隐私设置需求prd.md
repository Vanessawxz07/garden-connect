...
- **V2迭代**：支持数据驱动的机制化标签（如：交易次数达标、登录天数达标则授予“活跃用户”标签）
...
- **显示格式**：`● 当前在线` / `○ 离线`
- **在线判定**：
  - 使用心跳机制更新在线状态（用当前默认逻辑）
- **偏好在线时间**：用户设置常在线时间段（已有逻辑，更新展示位置）
...
#### 4.1.4 空状态
- **主人态**：显示"你还没有发起过任何抽奖活动"，提供「创建抽奖」按钮（若有VIP权限则可直接发起抽奖；如无抽奖功能权限，则按钮为拉起申请权限的表单入口）
